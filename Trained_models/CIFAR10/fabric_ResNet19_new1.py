...
def get_dataloaders(
    bs=64,
    item_tfms=[RandomResizedCrop(size=100, min_scale=0.35), FlipItem(0.5)],
...
def main():
    ...
    dls = get_dataloaders(bs=batch_size,
                      batch_tfms=[],
                      item_tfms=[Resize(size=100), FlipItem(0.5)]
                      )
    train_loader, test_loader = dls.train, dls.valid
    #train_loader, test_loader = fabric.setup_dataloaders(train_loader, test_loader)
    xb, yb = dls.valid.one_batch()
    print(xb.shape)
    model = resnet19_TEMPs(num_classes=100)
    epochs = 10
    lr = 1e-3 #5e-4
    optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-3) #(0.5 - 75.18)
    sched = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=lr, epochs=epochs, 
                                                steps_per_epoch=len(train_loader),
                                                pct_start=0.5,              # % of cycle spent increasing LR
    ...
    # torch.save(model.state_dict(),'/data/madhu/img100/ResNet19_IN100_100.pt') #_32 - 64 - 63%, _75 - 66, _100 - 70
